...
int getTotalAttack(int hit_mod, string weapontype)
{
	int str, attack_str, weap_skill, total_atk;
	str = query("stat/strength");
	printf("Strength = "+str);
	attack_str = query("attack_strength");
	printf(" Attack Strength = "+attack_str);
	weap_skill = query_skill(weapontype);
	printf(" Weapon Skill = " + weap_skill+"\n");
	total_atk = str + attack_str + weap_skill + hit_mod;
	if (total_atk < 5)
		return 5;
	else
		return total_atk;
...
int getTotalDefense(object defender)
{
	int dex, ac, def_skill, total_def;
	dex = defender->query("stat/dexterity");
    ac = defender->query("armor_class");
	if (!(int)defender->query_monster()==1) {
        if (defender->query("armor/shield")) {
            def_skill = defender->query_skill("Shield defense");
        } else {
            def_skill = defender->query_skill("Parrying defense");
        }
    } else {
        def_skill = defender->query_skill("defense");
    }
	printf("Defense = " + def_skill + " Armor = " + ac + " Dexterity = " + dex + "\n");
	total_def = dex + ac + def_skill;
	if (total_def < 0)
		return 0;
	else
		return total_def;
...
//          weapon = find_player(victim); //Leto
            hit_chance = member_array(weapon,attackers);
            if (hit_chance>-1) {
                attackers[hit_chance]=attackers[0];
                attackers[0]=weapon;
            } else {
...
//  if (!dam_mod) dam_mod=0;
	...
	total_atk = getTotalAttack(hit_mod,weapontype);
	total_def = getTotalDefense(attackers[0]);
	//total_def = dex + def_sk + ac;
	//printf(""+(string)(str + att_sk + wc + hit_mod)+"\n"); //+ hit_mod);
	//printf("Total_atk="+total_atk+"\n");
	//printf(""+(string)(dex + def_sk + ac)+"\n");
	printf("Attack: " + total_atk + " vs Defense: " + total_def + "\n");
	hit_chance = 70+(100*(total_atk)-100*(total_def))/(total_atk + total_def);
	printf(""+(string)hit_chance+"\n");
	//if (this_player()->query("wizard")){
	//message(MSG_DEBUG,sprintf("str="+str+" att_sk="+att_sk+" wc="+wc+" dex="+dex+" def_sk="+def_sk+" ac="+ac+" hit_mod="+hit_mod+" Total: "+hit_chance+"\n"),this_player());
	//}
     ...
     */
    damage = random(dmax-dmin+1)+dmin+str/10-1+att_sk/10-def_sk/10+dam_mod;
     ...
     */
	hitbox = random(MAX(hit_chance,100));
	printf("Hitbox = " + hitbox + " vs Hit_chance = " + hit_chance + "\n");
	if (hitbox < 6 && hit_chance < 100)
	{
		//straight glance
		damage = damage*3/4;
		printf("Glancing Hit.\n");		
	}
	else if (hit_chance > 100 && hit_chance < 106 && hitbox > 100 && hitbox < 106)
	{
		//reduced chance to glance
		damage = damage*3/4;
		printf("Glancing Hit.\n");	
	}
	else if (hitbox <= hit_chance && hitbox < 106)
	{
		//standard hit
	}
	else if (hitbox > 106)
    ...
    if (damage>0 && hitbox <= hit_chance){
        ...
        attackers[0]->receive_damage(damage);
